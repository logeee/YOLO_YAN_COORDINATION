...
cx           = 305.71
cy           = 268.34
dist-coeffs  = 0.05998239,-0.07112947,-0.00037432,0.00015172,0.01724672
...
服务启动默认值已经直接使用上面的标定内参；`260/320/240` 这组早期近似值已经不再作为当前 fallback。现场只有在重新标定后才需要通过 debug 页面或配置 API 更新运行时默认值。
...
早期按已知距离反推的近似 focal fallback 已经废弃，不再作为服务或脚本默认值。当前默认左目内参来自标准标定：`fx=275.06, fy=275.39, cx=305.71, cy=268.34, dist=[0.05998239,-0.07112947,-0.00037432,0.00015172,0.01724672]`。2026-05-27 起利群烟盒按 `280mm x 89mm` 自动切换尺寸。PnP 距离会随物体实际尺寸和相机内参一起缩放；如果之后有新的实测数据，更稳的做法是按不同类别分别测 2-3 个已知距离，再核对当前标定内参是否仍合适。
...
4. forward/back 会使用 control_distance_error_mm，它由转向后的预测近端边前向距离直接计算得到。
